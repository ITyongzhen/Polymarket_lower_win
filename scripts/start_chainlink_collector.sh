...
if [ -z "${PM_CHAINLINK_RUN_ID:-}" ] || [ "${PM_CHAINLINK_RUN_ID}" = "auto" ]; then
  export PM_CHAINLINK_RUN_ID="${PM_LOG_STAMP:-$(date +%Y%m%d%H%M%S)}"
fi

case "${PM_CHAINLINK_LOGS_ROOT:-}" in
  "" )
    export PM_CHAINLINK_LOGS_ROOT="logs/chainlink_streams"
    ;;
  Logs/*)
    export PM_CHAINLINK_LOGS_ROOT="logs/${PM_CHAINLINK_LOGS_ROOT#Logs/}"
    ;;
  Logs)
    export PM_CHAINLINK_LOGS_ROOT="logs/chainlink_streams"
    ;;
...
if [ -z "$PYTHON_BIN" ]; then
  echo "[chainlink] no supported Python found. Require Python 3.11+." >&2
  exit 2
fi
